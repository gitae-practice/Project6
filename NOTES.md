...
- **면접 진행 중 이전 면접관으로 돌아가기 기능 구현** (전날 메모해둔 작업) — 스텝 인디케이터에서
  ...
  아직 안 만난 면접관은 클릭 대상에서 제외(순서를 건너뛸 수 없게)
  - `goToInterviewer(index)` 신설 — 처음 만나는 면접관이면 기존 `startInterviewer()`로 첫
    질문을 받아오고, 이미 대화한 적 있는 면접관(뒤로 갔다 다시 앞으로 오는 경우 포함)이면
    `history[role].length === 0` 여부로 판단해서 기존 대화를 그대로 보여주기만 하고 새 질문으로
    덮어쓰지 않음 — 메모리에 남겨둔 주의사항("다음으로 다시 넘어갔을 때 기존 대화 덮어쓰지
    않기")을 정확히 이 분기로 처리함
  - `handleNextInterviewer()`도 내부적으로 `goToInterviewer()`를 타도록 변경해서 로직 일원화
  - 스트리밍 중에는 전환 못 하게 가드 처리
  - 스텝 표시 UI: 완료된 항목만 `<button>`으로 감싸고 나머지는 그대로 `<div>` — 동적 태그
    변수(`"button" | "div"`) 대신 조건부로 두 갈래 JSX를 분기해서 타입스크립트가 button 전용
    속성(`type`)을 div에도 허용해야 하는 상황을 피함
- tsc/lint/build 전부 통과 확인
