...
- **기본 테마 라이트모드 + 로그인 화면 테마 토글**
  - `ThemeToggle` 기본값을 시스템 다크모드 설정 무시하고 라이트로 고정 (localStorage에 저장된 값이 있으면 그 값 우선)
...
- **지원 직무 필수화 + 이력서 입력 검증 (불필요한 토큰 낭비 방지)** — 지원 직무를 필수 입력으로, 이력서는 PDF 업로드/텍스트 입력 중 최소 1개는 있어야 "시작하기" 가능하도록. 힌트 텍스트도 상황에 맞게 변경
