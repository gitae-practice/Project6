...
- **버그**: 위 방식이 레이아웃 단에서 매번 강제 리다이렉트하는 구조라, `/admin`의 "← 오늘의 면접관" 버튼을 눌러도 `/`로 갔다가 곧바로 다시 `/admin`으로 튕겨 나와 "아무 동작 안 하고 새로고침만 되는" 것처럼 보이는 문제 발생. 리다이렉트를 레이아웃(매 요청마다 강제)에서 `AuthForm`의 로그인 성공 시점(1회성)으로 옮겨서 해결 — 이후엔 admin 계정도 `/`로 자유롭게 이동 가능
