...
- **로그인 (Supabase Auth)** — 이메일/비밀번호 회원가입·로그인, 본인 세션만 조회 가능 (RLS)
- **3단계 면접 진행** — 기술 → 인성 → 압박 면접관 순서로 전환
- **지원 직무 / 이력서 입력(선택)** — 수기 입력과 PDF 업로드를 독립적으로 지원. PDF는 Claude가 직접 읽어 내용을 기억하고(요약 없이 원문 그대로), 수기 입력과 합쳐서 면접관 질문에 반영. 분석 완료 전에는 면접 시작 불가
